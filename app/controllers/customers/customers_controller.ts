...
export default class CustomersController {
  ...
  async index({ inertia }: HttpContext) {
    const overview = await customerService.getOverview()
  ...
  async store(ctx: HttpContext) {
    ...
    return runAction(ctx, () => customerService.create(actor, payload), {
  ...
  async update(ctx: HttpContext) {
    ...
    return runAction(ctx, () => customerService.update(actor, ctx.params.id, payload), {
